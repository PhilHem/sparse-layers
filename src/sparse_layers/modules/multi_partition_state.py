...
class SSEMultiPartitionStateConfig(BaseModel):
    ...
    num_partitions: int = Field(gt=1, description="Number of state partitions")
...
class NaiveMultiPartitionState(nn.Module):
    ...
    def update(self, partition_indices: Tensor, keys: Tensor, values: Tensor) -> None:
        """
    ...
    def _validate_update_inputs(
        ...
        if keys.dim() != 4:
            raise ValueError(f"Expected keys with shape (batch, seq_len, k, c), got {keys.shape}")

        if values.dim() != 3:
            raise ValueError(f"Expected values with shape (batch, seq_len, d), got {values.shape}")
        ...
        if keys.shape[3] != self.c:
            raise ValueError(f"Keys last dimension must match c={self.c}, got {keys.shape[3]}")
        ...
        if values.shape[2] != self.d:
            raise ValueError(f"Values last dimension must match d={self.d}, got {values.shape[2]}")
    ...
    def read(self, partition_indices: Tensor, queries: Tensor) -> Tensor:
        ...
        if seq_len == 0:
            return torch.zeros(
                batch, 0, queries.size(-1), dtype=queries.dtype, device=queries.device
            )
    ...
    def _validate_read_inputs(self, partition_indices: Tensor, queries: Tensor) -> None:
        ...
        if queries.dim() != 3:
            raise ValueError(
                f"Expected queries with shape (batch, seq_len, d), got {queries.shape}"
            )
    ...
    def extra_repr(self) -> str:
        return f"num_partitions={self.num_partitions}, c={self.c}, d={self.d}"
...
class SSEMultiPartitionState(nn.Module):
    ...
    def __init__(self, config: SSEMultiPartitionStateConfig) -> None:
        ...
        self.states = nn.Parameter(
            torch.zeros(config.num_partitions, config.c, config.d), requires_grad=False
        )
        self._forward_deltas: dict[int, Tensor] = {}
    ...
    def read(self, partition_indices: Tensor, queries: Tensor) -> Tensor:
        ...
        partition_outputs = gathered_states.mean(dim=3) * queries.unsqueeze(
            2
        )  # (batch, seq_len, k, d)
